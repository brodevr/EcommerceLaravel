...
<div class="max-w-lg">
    <div class="flex items-center gap-3 mb-6">
        ...
        <h1 class="text-2xl font-bold text-petfy-dark">Editar Categoría</h1>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow p-6">
        <form action="{{ route('admin.categorias.update', $category) }}" method="POST" class="space-y-4">
            ...
            <div>
                ...
                <p class="mt-1 text-xs text-slate-400">Slug actual: <span class="font-mono">{{ $category->slug }}</span></p>
            </div>

            <div>
                <label for="description" class="block text-sm font-medium text-petfy mb-1">Descripción</label>
                <textarea name="description" id="description" rows="3"
                          class="w-full border border-[#b2ebf2] rounded-lg px-3 py-2.5 focus:border-petfy focus:ring-2 focus:ring-petfy/25 focus:outline-none transition">{{ old('description', $category->description) }}</textarea>
